...
/// Spawns a single scan worker with its own datalink channel.
async fn scan_worker(
    ...
    let flags = crate::profile::get_scan_flags(&scan_type);
    let stats_rx = Arc::clone(&stats);
    let scan_type_owned = scan_type.clone();
    let scan_ports = ports.clone();
    let t_ip = target_ip;
    let shutdown_recv = Arc::clone(&shutdown);

    let recv_handle = tokio::task::spawn_blocking(move || {
        let mut found: HashSet<u16> = HashSet::new();
        let start = std::time::Instant::now();
        while start.elapsed() < Duration::from_secs(8) && !shutdown_recv.load(Ordering::Relaxed) {
            match _rx.next() {
                Ok(frame) => {
    ...
    let mut bypass = crate::bypass::BypassEngine::new("adaptive", 3);
    for (idx, &port) in ports.iter().enumerate() {
        if shutdown.load(Ordering::Relaxed) { break; }
        let s_ip = if source_vary { crate::profile::random_ip() } else { src_ip };
        let s_port = if random_ports { crate::profile::random_port() } else { port };
        ...
        if idx % 100 == 0 {
            ...
            let total = ports.len();
            print!("\rscan worker -> {} / {} | open: {} | sent: {} | drop: {}", idx + 1, total, open, sent, fail);
...
pub async fn run_port_scan(
    ...
    random_ports: bool,
    stats: Arc<PacketStats>,
    _bypass: &mut BypassEngine,
    shutdown: Arc<AtomicBool>,
    workers: usize,
) -> HashSet<u16> {
    if workers <= 1 {
        let result = scan_worker(iface.to_string(), src_mac, src_ip, dst_mac, target_ip,
            ports.to_vec(), scan_type.to_string(), random_opts, source_vary, random_ports,
            Arc::clone(&stats), Arc::clone(&shutdown)).await;
        println!("\nScan complete: {} open ports", result.len());
        for p in result.iter() { println!("  → {}", p); }
        return result;
    }

    let iface_owned = iface.to_string();
    let scan_type_owned = scan_type.to_string();
    let ports_owned = ports.to_vec();
    let chunk_size = (ports_owned.len() + workers - 1) / workers;
    let mut handles = Vec::new();

    for w in 0..workers {
        let start = w * chunk_size;
        let end = (start + chunk_size).min(ports_owned.len());
        if start >= ports_owned.len() { break; }
        let chunk = ports_owned[start..end].to_vec();
        let handle = tokio::spawn(scan_worker(
            iface_owned.clone(), src_mac, src_ip, dst_mac, target_ip,
            chunk, scan_type_owned.clone(), random_opts, source_vary, random_ports,
            Arc::clone(&stats), Arc::clone(&shutdown),
        ));
        handles.push(handle);
    }
    ...
    for h in handles {
        if let Ok(found) = h.await {
            all_found.extend(found);
        }
    }
    println!("\nScan complete: {} open ports", all_found.len());
    for p in all_found.iter() { println!("  → {}", p); }
    all_found
}

/// Spawns a single SYN flood worker with its own datalink channel.
async fn flood_worker(
    ...
    let (mut tx, mut rx) = match open_datalink(&iface, 100) {
        Some((tx, rx)) => (tx, rx),
        None => { tracing::error!("worker {} datalink channel: {}", worker_id, iface); return; }
    };
    ...
    tokio::task::spawn_blocking(move || {
        let start = std::time::Instant::now();
        while start.elapsed() < Duration::from_secs(duration + 5) && !shutdown_recv.load(Ordering::Relaxed) {
            match rx.next() {
                Ok(frame) => {
    ...
    while std::time::Instant::now() < deadline && !shutdown.load(Ordering::Relaxed) {
        for _ in 0..burst {
            let s_ip = if source_vary { crate::profile::random_ip() } else { src_ip };
            let s_port = if random_ports { crate::profile::random_port() } else { 31337 };
...
pub async fn run_syn_flood(
    ...
    for w in 0..workers {
        let ports_clone = ports_owned.clone();
        let handle = tokio::spawn(flood_worker(
            iface_owned.clone(), src_mac, src_ip, dst_mac, target_ip,
            ports_clone, connections, duration, source_vary, random_ports,
            Arc::clone(&stats), Arc::clone(&shutdown), w,
        ));
        handles.push(handle);
    }
    ...
    while report_start.elapsed() < Duration::from_secs(duration) && !shutdown.load(Ordering::Relaxed) {
        ...
        let workers_line = if workers > 1 { format!(" | workers: {}", workers) } else { String::new() };
        print!("\rflood -> sent: {} | drop: {} | syn-ack: {} | {:.0} pps{}", sent, fail, open, pps, workers_line);
        let _ = std::io::stdout().flush();
    }
